...
class AsynchronousLoader:
    ...
    def collect_batch_data(self, save_folder="/projects/I20240005/rnouaj/recovered_from_git/Results_workloads_deucalion/object_detection_speedy1/batch_composition"):
        ...
        while True:
            ...
            while len(batch_data) < self.batch_size:
                ...
                if not self.queue_timeout.empty():
                    ...
                    image, target, idx = item
                    item_with_tag = (image, target, idx, tag)
                    if not isinstance(image, torch.Tensor):
                        continue

                    batch_data.append(item_with_tag)
    ...
    def _move_to_pinned_memory(self, batch):
        pinned_batch = []
        image, target, idx , tag = batch  # Unpack tuple
        ...
        pinned_batch.append((image, target, idx, tag))  # Recreate tuple with pinned memory
        batch = (image, target, idx, tag)
    ...
    def __next__(self):
        ...
        while True:
            try:
                ...
                return self.batch_queue.get(timeout=0.05)
