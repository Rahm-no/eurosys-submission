...
class Compose(object):
    ...
    def __init__(self, transforms, queue_timeout=None,  timeouts = [0.019, 0.01, 0.013, 0.012] ):
        """
    ...
    def __call__(self, image, target):
        ...
        for i, (t, timeout) in enumerate(zip(self.transforms, self.timeouts)):
            ...
            if status == "Success":
                ...
                end_time = time.time()
                print(f"Time taken to put in task queue: {end_time - start_time:.6f} seconds")
                return None, None
    ...
    def process_remaining_transforms(self):
        ...
        while True:
            ...
            print(f"Time taken to get from task queue: {get_duration:.6f} seconds")
            if item is None:
        ...
        if get_count > 0:
            ...
            print(f"Average time to get item from task queue: {avg_get_time:.6f} seconds")
        else:
            print("No items were processed from the task queue.")
    def __repr__(self):
