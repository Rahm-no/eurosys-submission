...
plt.xlabel('Number of long samples in a batch')
plt.ylabel('Proportion of batches')
plt.title('Batch Composition Categories (by #Long per Batch)')
plt.legend()
...
for gpu, ax in zip(sorted(series['gpu_id'].unique()), axes):
    for loader, sub in series[series['gpu_id']==gpu].groupby('loader'):
        ax.plot(sub['batch_id'], sub['num_long'], label=loader, alpha=0.9)
        avg_frac = sub['num_long'].mean()
        ax.axhline(y=avg_frac, color='r', linestyle='--', 
                   label=f"{loader} avg = {avg_frac:.2f}")
    ax.set_title(f"GPU {gpu}")
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.set_ylabel("num long")

axes[-1].set_xlabel("Iteration")
axes[0].legend()

ymax = series['num_long'].max()
ytop = np.ceil(ymax * 10) / 10.0
for ax in axes:
    ax.set_ylim(0, ytop)
