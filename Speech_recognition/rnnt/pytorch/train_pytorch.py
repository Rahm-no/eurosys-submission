...
def parse_args():
    ...
    io.add_argument('--val_frequency', default=5, type=int,
...
def evaluate(epoch, step, val_loader, val_feat_proc, detokenize,
             ema_model, loss_fn, greedy_decoder, use_amp, device, val_augmentations):

    ema_model.eval()
    start_time = time.time()
    agg = {'losses': [], 'preds': [], 'txts': [], 'idx': []}

    pipeline_type = getattr(val_loader, 'pipeline_type', 'torch')
    logging.log_start(logging.constants.EVAL_START, metadata=dict(epoch_num=epoch))

    for i, batch in enumerate(val_loader):
        print(f'{pipeline_type} evaluation: {i:>10}/{len(val_loader):<10}', end='\r')
        ...
        # === Step 1: Extract features (manually or via val_feat_proc) ===
        # if val_feat_proc is not None:
        #     feats, feat_lens = val_feat_proc([audio, audio_lens])
        #     feats = feats.to(device)
        #     feat_lens = feat_lens.to(device)
        # else:
        #     # Minimal placeholder: assume audio is already [B, T, 240]
        feats = audio
        feat_lens = audio_lens
        ...
        # === Step 4: Prepare input for decoder ===
        # Expected: x → [B, C, F, T] where feats: [B, T, F]
        # Inside model forward or encode:



        feats_for_decode = feats
        x= feats
        if x.shape[-1] != 240:
            x = torch.nn.functional.linear(x, torch.randn(240, x.shape[-1], device=x.device))
        pred = greedy_decoder.decode(ema_model, feats_for_decode, feat_lens)
    ...
    # === Final step: Compute metrics and save ===
    wer, loss = process_evaluation_epoch(agg)

    with open("acuracy_speech.csv", "a") as f:
        writer = csv.writer(f)
        writer.writerow([epoch, wer, loss])
...
def main():
    ...
    train_dataset = AudioDataset(data_dir=args.dataset_dir, manifest_fpaths=args.train_manifests, tokenizer=tokenizer, normalize_transcripts=True)
    val_dataset = AudioDataset(data_dir=args.dataset_dir, manifest_fpaths=args.val_manifests, tokenizer=tokenizer, normalize_transcripts=True)

    train_loader = get_data_loader(train_dataset, batch_size=batch_size, world_size=world_size, rank=args.local_rank)
    val_loader = get_data_loader(val_dataset, batch_size = batch_size, world_size= world_size, rank = args.local_rank)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch in train_loader:
            iteration_start_time = time.time()  # Start time of iteration
            ...
            print("audio in train pytorch", audio.shape)
            print("audio_lens", audio_lens.shape)
            print("txt", txt.shape)
            print("txt_lens", txt_lens.shape)
            print("model forward")
           ...
           # Ensure both logits and labels are sliced to match the target length
            ...
            n+=1
            accumulated_batches = 0
            if n % args.val_frequency == 0:
                print("step", step, "inside evaluation")
                wer = evaluate(epoch, step, val_loader, val_feat_proc,
                            tokenizer.detokenize, ema_model, loss_fn,
                            greedy_decoder, args.amp, device, val_augmentations)
...
if __name__ == "__main__":
    ...
    throughput_file = '/projects/I20240005/rnouaj/recovered_from_git/Results_workloads_deucalion/Speech_recognition/rnnt/pytorch/throughput__resultsAe.csv'
