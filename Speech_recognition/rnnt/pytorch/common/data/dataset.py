...
def preprocess_audio1(waveform, n_fft=400, hop_length=160, n_mels=240):
    ...
    print(f"Preprocessing took {preprocessing_time:.2f} seconds")
...
class AudioDataset(Dataset):
    ...
    def preprocess_audio(self, segment):
        ...
        if self.sample_count % 5 == 0:
            ...
            print("Regular light work processing")
    ...
    def heavy_processing(self, segment):
        ...
        while time.time() - heavy_work_start < 4:
            _ = sum(i for i in range(10000))  # Simulate more intensive computational work
    ...
    def __getitem__(self, index):
        ...
        segment = AudioSegment(
            s['audio_filepath'][rn_indx], target_sr=8000,
...
def get_data_loader(dataset, batch_size, world_size, rank, shuffle=True,
                    drop_last=True, num_workers=12, num_buckets=None):
    if world_size != 1:
        ...
        if num_buckets:
            ...
            print("length of sampler", len(sampler), "world_size", world_size, "dataset", len(dataset))
