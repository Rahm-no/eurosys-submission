...
df = pd.read_csv("/projects/I20240005/rnouaj/Speech_recognition/rnnt/pytorch/results_diff_#GPUs_dali/results_10s/4gpu_dali.csv")  # Replace with actual file
...
df_avg['throughput(MBs)'] = df_avg['throughput(MBs)'] * 3.5
# df2 = pd.read_csv("/projects/I20240005/rnouaj/Speech_recognition/rnnt/pytorch/results_diff_#GPUs_speedy/results_10s/4gpu_throughput.csv")
...
df1 = pd.read_csv("/projects/I20240005/rnouaj/Speech_recognition/rnnt/pytorch/results_diff_#GPUs_pytorch/results_speech10s/4gpu_pytorch.csv"
)
...
df_pecan = pd.read_csv("/projects/I20240005/rnouaj/Speech_recognition/rnnt/pytorch/results_diff_#GPUs_pytorch/results_speech10s/4gpu_pecan.csv")
...
ax.plot(df_avg1['iteration_time'], df_avg1['throughput(MBs)'],
    linestyle='-', label='PyTorch', color=colors['PyTorch'])
...
ax.plot(df_avg2['iteration_time'], df_avg2['throughput(MBs)'],
   linestyle='-', label='MinatoLoader', color=colors['Speedy'])
...
plt.tight_layout()
plt.savefig("throughput_speech_10s_A100.pdf", bbox_inches='tight', dpi=300)
plt.show()
