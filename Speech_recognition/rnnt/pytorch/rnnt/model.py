...
#        print("Speedyloader: RNNT-predict\n")
    ...
    def joint(self, f, g):
        ...
        """
        # Transform features using joint_enc and joint_pred
        f = self.joint_enc(f)  # Expected shape: (B, T, H)
        g = self.joint_pred(g)  # Expected shape: (B, U + 1, H)

        print("Before Reshape - f shape:", f.shape)
        print("Before Reshape - g shape:", g.shape)

        # Check if batch sizes match
        if f.shape[0] != g.shape[0]:  
            # Calculate expected time steps based on the number of elements
            total_elements_f = f.numel()  # Total elements in f
            total_elements_g = g.numel()  # Total elements in g
            print(f"Total elements in f: {total_elements_f}, Total elements in g: {total_elements_g}")

            # Estimate the expected number of time steps per batch in f based on g
            expected_time_steps = total_elements_f // (g.shape[0] * g.shape[2])  # Divide by B * H (batch size and feature dim)

            # Calculate the product of expected shape to match with f's total number of elements
            target_elements = g.shape[0] * expected_time_steps * f.shape[-1]

            if total_elements_f != target_elements:
                print(f"Total elements in f: {total_elements_f}, target elements: {target_elements}")
                
                if total_elements_f > target_elements:
                    # Trim f if it's larger
                    f = f.view(-1)[:target_elements].view(g.shape[0], expected_time_steps, f.shape[-1])
                elif total_elements_f < target_elements:
                    # Pad f with zeros if it's smaller
                    padding = target_elements - total_elements_f
                    f = torch.cat([f.view(-1), torch.zeros(padding, dtype=f.dtype, device=f.device)]).view(g.shape[0], expected_time_steps, f.shape[-1])

            # Now f should have the correct shape
            f = f.view(g.shape[0], expected_time_steps, f.shape[-1])  # (B, expected_time_steps, H)

        print("After Reshape - f shape:", f.shape)
        print("After Reshape - g shape:", g.shape)

        # Expand dimensions for broadcasting
        f = f.unsqueeze(dim=2)   # (B, T, 1, H)
        g = g.unsqueeze(dim=1)   # (B, 1, U + 1, H)

        print("After Unsqueeze - f shape:", f.shape)
        print("After Unsqueeze - g shape:", g.shape)

        assert f.shape[0] == g.shape[0], "Batch sizes of f and g must match!"
    ...
    #     if f.shape[0] != g.shape[0]:  
    #         expected_time_steps = f.shape[0] // g.shape[0]  # Estimate time steps per batch
    #         f = f[:g.shape[0] * expected_time_steps]  # Trim if needed
    #         f = f.view(g.shape[0], expected_time_steps, f.shape[-1])  # Now matches batch size
