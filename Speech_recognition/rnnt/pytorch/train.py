...
# throughput_file = 'throughput_log_dali_10s.csv'

# with open(throughput_file, 'w', newline='') as csvfile:
#     csv_writer = csv.writer(csvfile)
    
 
#     csv_writer.writerow(['Iteration', 'time', '#iter/s'])
...
def calculate_tensor_size(tensor):
    ...
    print("tensor shape", tensor.shape, "num_elements", num_elements, "bytes_per_element", bytes_per_element)
...
def main():
    ...
    def print_shape_hook(module, input, output):
        ...
        output_shapes = output.shape if isinstance(output, torch.Tensor) else [out.shape for out in output if isinstance(out, torch.Tensor)]
        
        print(f"Layer: {module.__class__.__name__} | Input Shapes: {input_shapes} | Output Shape: {output_shapes}")


    # Register hooks for all transformations
    for layer in train_augmentations:
    ...
    print("steps_per_epoch dali", steps_per_epoch)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            size = size + calculate_tensor_size(feats) + calculate_tensor_size(feat_lens) + calculate_tensor_size(txt) + calculate_tensor_size(txt_lens)

            if time.time() - last_time >= 5:
                # Calculate number of iterations per second over the last 5 seconds
                iter_persec = (n - last_step) / (time.time() - last_time)
                throughput = (size / (time.time() - last_time) )   #11xx
