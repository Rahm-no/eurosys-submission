...
class COCODataset(torchvision.datasets.coco.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if self._transforms is not None:
            img, target, tags = self._transforms(img, target)

        return img, target, idx, tags
...
class Compose(object):
    def __init__(self, transforms):
        self.transforms = transforms
        self.timeouts = [0.023, 0.02, 0.023, 0.022]
    ...
    def __call__(self, image, target):
        ...
        for i, (t, timeout) in enumerate(zip(self.transforms, self.timeouts)):
            ...
            if duration > timeout:
                print(f"Warning: Transform {i} ({t}) took too long: {duration:.4f}s, exceeding timeout of {timeout:.4f}s")
                tag ='L'
         

            

        return image, target, tag
