    ...
    try:    
        while not stop_event.is_set():
            ...
            while len(batch_data) < batch_size:
                ...
                if not queue_timeout.empty():
                    item, rank = queue_timeout.get()
                    print('item from queue_timeout')
                elif not queue.empty(): # If queue is empty, dequeue from timeout queue
                    item, rank = queue.get()
...
def main():
    ...
    print("device in here is what", device)
    ...
    train_dataset = AudioDataset(data_dir=args.dataset_dir, manifest_fpaths=args.train_manifests, tokenizer=tokenizer,  queue = queue, queue_timeout = queue_timeout,  rank=args.local_rank, normalize_transcripts=True, heavy_fraction=0.25)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        while True:
            ...
            if not batch_queue.empty():
                print("batch_queue is not empty")
                batch = batch_queue.get()
            else:
                print("batch_queue is empty, waiting for data")
                time.sleep(0.1)
                continue
...
if __name__ == "__main__":
    ...
    throughput_file = 'results/outputs_fraction/fraction25/speedy_lfrac250.csv'
