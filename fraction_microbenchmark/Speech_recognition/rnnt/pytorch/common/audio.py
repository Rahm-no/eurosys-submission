...
def preprocess_audio(samples, sample_rate, n_mels=240):
    mel_spectrogram = librosa.feature.melspectrogram(
        y=samples.numpy(),  # Convert to NumPy array
        sr=sample_rate,
        n_mels=n_mels
    )
    log_mel_spectrogram = librosa.power_to_db(mel_spectrogram, ref=np.max)
    return torch.tensor(log_mel_spectrogram.T, dtype=torch.float32)  # Transpose for time_steps x n_mels
...
def audio_from_file(file_path, offset=0, duration=0, trim=False, target_sr=16000):
    ...
    # Preprocess audio to have 240 features in the last dimension
    features = preprocess_audio(samples.unsqueeze(0), audio.sample_rate)  # (time_steps, 240)
    return features, num_samples.unsqueeze(0)
...
class AudioSegment(object):
    ...
    def __init__(self, filename, target_sr=None, int_values=False, offset=0,
                 duration=0, trim=True, trim_db=60):
        ...
        with sf.SoundFile(filename, 'r') as f:
            dtype = 'int32' if int_values else 'float64'
        ...
        self._samples  = self.pad(self._samples, 800, symmetric=True)



        samples = self._convert_samples_to_float32(samples)
        print("samples type after convert samples to float32 called : ", samples.dtype)
        print("samples shape after convert samples to float32 called: ", samples.shape)


        if target_sr is not None and target_sr != sample_rate: ## WE CAN put it at the begiining for pecan
            samples = librosa.core.resample(samples, sample_rate, target_sr)
            sample_rate = target_sr
            print("samples type after resample called : ", samples.dtype)
            print("samples shape after resample called: ", samples.shape)
        if trim:
            print("samples type before trim called : ", samples.dtype)
            print("samples shape before trim called: ", samples.shape)
            samples, _ = librosa.effects.trim(samples, trim_db)
            print("samples type after trim called : ", samples.dtype)
            print("samples shape after trim called: ", samples.shape)
        ...
        print("samples shape before pad: ", samples.shape) 
        #pecan and pytorch pad transformation
        # self._samples  = self.pad(self._samples, 800, symmetric=True)
        ...
        print("samples shape after pad: ", self._samples.shape)
    ...
    def pad(self, samples, pad_size, symmetric=False):
