...
def additional_prep(waveform, n_fft=400, hop_length=160, n_mels=240):
    """
...
class AudioDataset(Dataset):
    def __init__(self, data_dir, manifest_fpaths,
                 tokenizer, queue, queue_timeout,rank, heavy_fraction=0.2,
        ...
        self.num_workers = 2  # Adjust based on CPU availability
        self.queue_swap = mp.Queue()  # Shared queue for processing
        self.worker_processes = []
        for i in range(self.num_workers):
            p = mp.Process(target=self.worker_function, args=(self.queue_swap,))
            p.daemon = True  # Ensure it closes when main program exits
            p.start()
            self.worker_processes.append(p)
        self.heavy_fraction = heavy_fraction   # e.g., 0.0, 0.25, 0.5, 0.75, 1.0
        ...
        self.sample_count = 0  # Track how many samples processed
        self.samples = []
        ...
        # List to store active heavy processes
    ...
    def preprocess_audio(self, segment, s):
        self.sample_count += 1

        if self.sample_count % 4 == 0:

            print(f"Heavy processing added at sample AudioDataset {self.sample_count}!")
            self.queue_swap.put((segment, s))
            return "heavy"
        else:
    ...
    def light_processing(self, segment):
        """Simulate light work worth ~0.5 seconds"""
        start_time = time.time()
        while time.time() - start_time < 0.5:
            _ = sum(i for i in range(1000))  # Simulate light computational work
        print(f"Light preprocessing time for sample {self.sample_count} completed.")

    
    def worker_function(self, queue_swap):
        """Worker process that continuously processes heavy tasks."""
        while True:
            try:
                segment, s = queue_swap.get(timeout=5)
                self.heavy_processing(segment, s)
            except Empty:
                print("Worker timed out waiting for data. Continuing...")
                continue
            except Exception as e:
                print(f"Worker encountered an error: {e}")
                time.sleep(1)

    def heavy_processing(self, segment, s):
        """Simulate heavy work for every 5th sample."""
        heavy_work_start = time.time()
        while time.time() - heavy_work_start < 3:
            _ = sum(i for i in range(1000))  # Simulate intensive computational work
        batch = (segment,
        ...
        while self.queue_timeout.qsize() > 97:
                time.sleep(0.01)  # Small sleep to allow processing
        self.queue_timeout.put((batch, self.rank))
        print("Added to queue timeout", self.queue_timeout.qsize())
    ...
    def __getitem__(self, index):
        ...
        start_time = time.time()
        segment = self.preprocess_audio(segment, s)
        if segment == 'heavy':
            return 'heavy'
...
def get_data_loader(dataset, batch_size, world_size,queue, queue_timeout, shards, rank, device,shuffle=True,
    ...
    return AsynchronousLoader(dataset, 
    queue=queue, 
    queue_timeout=queue_timeout,
    device=device,  shards = shards, 
    sampler=sampler,
