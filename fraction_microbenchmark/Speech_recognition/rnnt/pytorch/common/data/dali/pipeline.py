...
import cupy as cp
import random
def gpu_delay(x, idx, heavy_fraction=0.75, heavy_ms=300, light_ms=50):
    n = int(1 / heavy_fraction)  # e.g. 1/0.25 = 4
    duration_ms = heavy_ms if (idx % n == 0) else light_ms

    _simulate_gpu_work(duration_ms)
import cupy as cp

def _simulate_gpu_work(duration_ms):
    ...
    while True:
        ...
        if cp.cuda.get_elapsed_time(start, end) >= duration_ms:
            break
