...
class AudioDataset(Dataset):
    def __init__(self, data_dir, manifest_fpaths,
                 ...
                 heavy_fraction=0.0,  # fraction of heavy samples
                 deterministic=True,   # control if deterministic vs random per sample
        ...
        assert 0.0 <= heavy_fraction <= 1.0, "heavy_fraction must be between 0 and 1"
        ...
        # --- Precompute heavy/light assignment
        n_total = len(self.samples)
        n_heavy = int(round(n_total * heavy_fraction))
        indices = np.arange(n_total)

        if deterministic:
            self.rng.shuffle(indices)
            self.heavy_indices = set(indices[:n_heavy])  # exactly n_heavy samples
        else:
            self.heavy_indices = None  # fallback to probabilistic at runtime

        print(f"[Init] Heavy fraction={heavy_fraction}, "
              f"Heavy samples={n_heavy}/{n_total}")

    def preprocess_audio(self, segment, index):
        self.sample_count += 1

        if self.deterministic and self.heavy_indices is not None:
            if index in self.heavy_indices:
                self.heavy_processing(segment)
            else:
                self.light_processing(segment)
        else:
            # fallback probabilistic
            if self.rng.rand() < self.heavy_fraction:
                self.heavy_processing(segment)
            else:
                self.light_processing(segment)
    ...
    def light_processing(self, segment):
        ...
        print("Light preprocessing done.")
    ...
    def heavy_processing(self, segment):
        ...
        print("Heavy preprocessing done.")
    ...
    def __getitem__(self, index):
        ...
        # Apply preprocessing (deterministic or probabilistic)
        self.preprocess_audio(segment, index)
