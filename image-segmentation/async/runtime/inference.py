    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(loader, disable=(rank != 0) or not flags.verbose)):
            ...
            with autocast(enabled=flags.amp):
                ...
                # print('EVAL LOSS VALUE', eval_loss_value)
                # print("score value", score_fn(output, label))
...
def gaussian_kernel(n, std):
    ...
    gaussian1D = norm.pdf(x, loc=0, scale=std)
    gaussian1D /= gaussian1D.max()  # Normalize

    gaussian2D = np.outer(gaussian1D, gaussian1D)
    gaussian3D = np.einsum('ij,k->ijk', gaussian2D, gaussian1D)  # Equivalent to outer product in 3D
    gaussian3D = np.cbrt(gaussian3D)  # Cube root normalization
    gaussian3D /= gaussian3D.max()

    return torch.from_numpy(gaussian3D).float()
