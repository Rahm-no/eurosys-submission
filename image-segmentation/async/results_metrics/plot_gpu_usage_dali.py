...
# Load the log file
df = pd.read_csv('/projects/I20240005/rnouaj/image-segmentation/dali/Baselines_Speedy/mlcomns_imseg_with_dali/results_metrics/GPU_dali_test2.log',delim_whitespace=True)
...
new_time = np.linspace(5, np.max(df["Time(s)"]), num=308).astype(int)
...
plt.savefig('dali_imseg_gpuusage.pdf', bbox_inches="tight", dpi=300)
plt.tight_layout()
plt.show()
