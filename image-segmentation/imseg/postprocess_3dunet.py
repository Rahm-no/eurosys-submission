...
for i, (name, sub) in enumerate(prop.groupby('loader')):
    ...
    plt.bar(x + (i - 0.5) * width, aligned, width=width, label=name, color = color)
...
plt.tight_layout()
plt.savefig(out_dir / 'fig_batchcomp3dunet_categories.pdf', dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(out_dir / f"fig_batchcomp_3dunet_timeline_{iter_min}_{iter_max}.pdf", dpi=300)
plt.close()
